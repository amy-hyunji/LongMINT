...
class CacheOpenAI(BaseLLM):
    ...
    def _init_llm_config(self) -> None:
        ...
        # Qwen3 family chat templates default `enable_thinking=True`. For
        # NER / triple extraction / QA on this codebase we want short, decisive
        # answers — the <think>...</think> trace bloats every call by 30-60s and
        # blows the 5-minute httpx timeout on heavy inputs. Pass through vLLM's
        # extra_body so the server-side template flag is honored.
        if config_dict.get("llm_disable_thinking", False):
            generate_params["extra_body"] = {
