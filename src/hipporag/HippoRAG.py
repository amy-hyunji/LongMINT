...
class HippoRAG:
    ...
    def __init__(self,
        ...
        self.tokenize = AutoTokenizer.from_pretrained(llm_model_name)
    ...
    def initialize_graph(self):
        ...
        # if not self.global_config.force_index_from_scratch:
            ...
            #     print(f"\n\n!!!Loading from preexisting graph pickle!!!\n\n")
    ...
    def index(self, docs: List[str]):
        ...
        logger.info(f"Indexing Documents")
        # print(f"Index Documents - {docs}")
        # print(f"[Index] # of Index Documents - {len(docs)}")
        ...
        # print(f"[Before] Chunk embedding store had elements - {self.chunk_embedding_store.get_num()}")
        self.chunk_embedding_store.insert_strings(docs)
        # print(f"[After] Chunk embedding store had elements - {self.chunk_embedding_store.get_num()}")
        ...
        if len(chunk_keys_to_process) > 0:
            new_ner_results_dict, new_triple_results_dict = self.openie.batch_openie(new_openie_rows)
            # print(f"new_ner_results_dict: {new_ner_results_dict}")
            # print(f"new_triple_results_dict: {new_triple_results_dict}")
            self.merge_openie_results(all_openie_info, new_openie_rows, new_ner_results_dict, new_triple_results_dict)
        # else:
        #     print(f"No new chunks to process for OpenIE.")
        ...
        # Filter results to only the current chunk ids to keep dictionaries aligned with the
        # embeddings just inserted in this indexing batch. This does not delete any persisted
        # OpenIE history; full results remain saved on disk.
        current_chunk_ids = set(chunk_to_rows.keys())
        ner_results_dict = {cid: ner_results_dict[cid] for cid in ner_results_dict if cid in current_chunk_ids}
        triple_results_dict = {cid: triple_results_dict[cid] for cid in triple_results_dict if cid in current_chunk_ids}
        ...
        if num_new_chunks > 0:
            ...
            logger.info(f"Found 0 new chunks to save into graph.")
            assert False
    ...
    def retrieve(self,
        ...
        for q_idx, query in enumerate(queries):
            ...
            if len(top_k_facts) == 0:
                # print('No facts found after reranking, return DPR results')
                sorted_doc_ids, sorted_doc_scores = self.dense_passage_retrieval(query)
            else:
                # print('Running graph_search_with_fact_entities')
            ...
            # print(f"Num_to_retrieve - {num_to_retrieve}\nretrieved num: {len(top_k_docs)}\n# of elements in chunk_embedding_store: {self.chunk_embedding_store.get_num()}\n# of elements in entity_embedding_store: {self.entity_embedding_store.get_num()}")
            # print(f"len(sorted_doc_ids): {len(sorted_doc_ids)}")
            # print(f"sorted_doc_ids: {sorted_doc_ids}")
            # print(f"len(self.passage_node_keys): {len(self.passage_node_keys)}")
    ...
    def rag_qa(self,
        ...
        # Performing QA
        queries_solutions, all_response_message, all_metadata, input_tokens_list, output_tokens_list = self.qa(queries, prompt=prompt, answer_options=answer_options)
        ...
        if gold_answers is not None:
            ...
            return queries_solutions, all_response_message, all_metadata, overall_retrieval_result, overall_qa_results, input_tokens_list, output_tokens_list
        else:
            return queries_solutions, all_response_message, all_metadata, input_tokens_list, output_tokens_list
    ...
    def qa(self, queries: List[QuerySolution], prompt=None, answer_options=None) -> Tuple[List[QuerySolution], List[str], List[Dict]]:
        """
        ...
        for query_solution in queries:
            ...
            # Optional: cap user-prompt size before LLM dispatch. With wiki-style
            # corpora a single retrieved passage can be 14k+ tokens, easily
            # overflowing the model's context. We trim from the docs tail so the
            # question is preserved verbatim and the highest-scoring evidence
            # stays intact. Tracked per-query so callers can see what was cut.
            qa_max_input_chars = getattr(self.global_config, "qa_max_input_chars", 0)
            input_truncated = False
            input_orig_chars = len(prompt_user)
            if qa_max_input_chars and input_orig_chars > qa_max_input_chars:
                cut = "\nQuestion:"
                if cut in prompt_user:
                    qi = prompt_user.rindex(cut)
                    docs_part, q_part = prompt_user[:qi], prompt_user[qi:]
                    if len(q_part) >= qa_max_input_chars:
                        prompt_user = q_part[-qa_max_input_chars:]
                    else:
                        prompt_user = docs_part[: qa_max_input_chars - len(q_part)] + q_part
                else:
                    prompt_user = prompt_user[-qa_max_input_chars:]
                input_truncated = True
            query_solution.input_truncated = input_truncated
            query_solution.input_orig_chars = input_orig_chars
            query_solution.input_sent_chars = len(prompt_user)
            ...
            # if first:
            #     print("="*80)
            #     print(f"retrieved_passages: {retrieved_passages}")
            #     print("="*80)
            #     print(f"prompt: {prompt_user}")
            #     print("="*80)
            #     first = False

            prompt_list.append(prompt_user)
            all_qa_messages.append(
                self.prompt_template_manager.render(name=f'rag_qa_{prompt_dataset_name}', prompt_user=prompt_user))

        # Fan QA calls out across a thread pool — vLLM batches concurrent requests
        # server-side, so this is the main throughput lever for QA. Override with
        # HIPPORAG_QA_CONCURRENCY=N. cache_response uses FileLock + per-call sqlite
        # connections, so it is thread-safe.
        qa_concurrency = int(os.environ.get("HIPPORAG_QA_CONCURRENCY", "32"))
        if qa_concurrency > 1 and len(all_qa_messages) > 1:
            with ThreadPoolExecutor(max_workers=min(qa_concurrency, len(all_qa_messages))) as pool:
                all_qa_results = list(pool.map(self.llm_model.infer, all_qa_messages))
        else:
            all_qa_results = [self.llm_model.infer(qa_messages) for qa_messages in all_qa_messages]
        ...
        for query_solution_idx, query_solution in enumerate(queries):
            response_content = all_response_message[query_solution_idx]
            # Strip Qwen-style <think>…</think> reasoning blocks before parsing.
            stripped = re.sub(r"<think>.*?</think>", "", response_content, flags=re.DOTALL)
            if "<think>" in stripped and "</think>" not in stripped:
                stripped = stripped.split("<think>")[0]
            try:
                tail = stripped.split('Answer:')[1]
                # Reasoning models (Qwen3.6) often write the answer correctly
                # then continue commentary ("Timwi.\nProceed.\nOne minor point…").
                # The convention from the rag_qa_musique one-shot is that the
                # answer occupies a single line, so cut at the first newline.
                pred_ans = tail.split('\n', 1)[0].strip()
                # Drop a wrapping period the LLM sometimes appends.
                if pred_ans.endswith('.'):
                    pred_ans = pred_ans[:-1].rstrip()
            except Exception as e:
                logger.warning(f"Error in parsing the answer from the raw LLM QA inference response: {str(e)}!")
                pred_ans = stripped.strip()

            query_solution.answer = pred_ans
            responses.append(pred_ans)
            # if query_solution_idx == 0:
            #     print(f"Answer - {pred_ans}")
            queries_solutions.append(query_solution)

        # Initialize lists to store input and output tokens
        input_tokens_list = []
        output_tokens_list = []

        # Process each question and response
        for qtext, response in zip(prompt_list, responses):
            
            # Tokenize the question and response using the tokenizer
            input_token_ids = self.tokenize(qtext, return_tensors=None)['input_ids']
            output_token_ids = self.tokenize(response, return_tensors=None)['input_ids']

            # Append the token counts to the lists
            input_tokens_list.append(len(input_token_ids))
            output_tokens_list.append(len(output_token_ids))

        # Log the token counts
        logger.info(f"Input tokens: {input_tokens_list}")
    ...
    def add_synonymy_edges(self):
        ...
        for node_key in query_node_key2knn_node_keys.keys():
            synonyms = []
    ...
    def get_fact_scores(self, query: str) -> np.ndarray:
        ...
        # print(f"Get Fact Scores - {len(self.fact_embeddings)}")
    ...
    def get_top_k_weights(self,
        ...
        # --- NEW: also zero out any non-zero weights that don't belong to known phrase nodes ---
        mapped_ids = set(self.node_name_to_vertex_idx.values())
        for idx in np.nonzero(all_phrase_weights)[0]:
            if idx not in mapped_ids:
                all_phrase_weights[idx] = 0.0
        # ---------------------------------------------------------------------------

        # Some entries in linking_score_map may correspond to phrases that aren't
        # represented as vertices in the KG (linking returned them via embedding
        # similarity, but OpenIE didn't yield a node). Their phrase weights stay
        # zero, so we drop them from linking_score_map to keep the two in sync
        # rather than asserting strict equality.
        kept = {
            phrase: s for phrase, s in linking_score_map.items()
            if compute_mdhash_id(content=phrase, prefix="entity-") in self.node_name_to_vertex_idx
        }
        linking_score_map = kept
        # Sanity check after the filter: counts must now match.
        assert np.count_nonzero(all_phrase_weights) == len(linking_score_map.keys()), f"np.count_nonzero(all_phrase_weights): {np.count_nonzero(all_phrase_weights)}, len(linking_score_map.keys()): {len(linking_score_map.keys())}"
    ...
    def rerank_facts(self, query: str, query_fact_scores: np.ndarray) -> Tuple[List[int], List[Tuple], dict]:
        ...
        if len(query_fact_scores) == 0 or len(self.fact_node_keys) == 0:
            print("No facts available for reranking. Returning empty lists.")
