...
def _extract_ner_from_response(real_response):
    ...
    if match is None:
        ...
        return []
    try:
        result = eval(match.group()).get("named_entities", [])
    except Exception:
        # eval can raise on malformed JSON-ish output; treat as no entities.
        return []
    if not isinstance(result, list):
        return []
    # Filter to non-empty strings. Some LLMs (Qwen3.6 in particular) emit a
    # literal `...` (Python Ellipsis) inside the entity list when they want to
    # signal "and more"; eval() returns Ellipsis here, which then crashes the
    # downstream json.dumps() in triple_extraction.
    ...
    def triple_extraction(self, chunk_key: str, passage: str, named_entities: List[str]) -> TripleRawOutput:
        def _extract_triples_from_response(real_response):
            ...
            if match is None:
                ...
                return []
            try:
                result = eval(match.group()).get("triples", [])
            except Exception:
                return []
            if not isinstance(result, list):
                return []
            # Same Ellipsis / non-list guard as NER. Each triple should be a
            # 3-element list/tuple of strings; filter_invalid_triples below
            # handles shape, but we defensively drop non-iterables here so
            # later len()/indexing doesn't crash.
            return [t for t in result if isinstance(t, (list, tuple))]
