...
class BaseConfig:
    ...
    qa_max_input_chars: int = field(
        default=0,
        metadata={"help": "When > 0, truncate the QA user prompt at this many "
                          "chars before sending to the LLM. Trims docs from the "
                          "tail (worst-scoring last) so the question is preserved. "
                          "Per-query truncation status is attached to each "
                          "QuerySolution as `input_truncated`/`input_orig_chars`/"
                          "`input_sent_chars`. 0 = disabled."}
    )
    llm_disable_thinking: bool = field(
        default=False,
        metadata={"help": "If True, pass extra_body={'chat_template_kwargs': "
                          "{'enable_thinking': False}} to every chat completion. "
                          "For Qwen3 family this skips the <think>...</think> "
                          "trace, dramatically speeding up NER / triple extraction "
                          "/ QA when only a short answer is needed."}
    )
