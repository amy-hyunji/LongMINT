...
# Compat shim: vllm 0.8.5 caches `tokenizer.all_special_tokens_extended` at
# init time, but transformers 5.x removed that attribute. Alias it to
# `all_special_tokens` so vllm's tokenizer caching can proceed.
...
if not hasattr(_PTTB, "all_special_tokens_extended"):
    _PTTB.all_special_tokens_extended = property(lambda self: list(self.all_special_tokens))
