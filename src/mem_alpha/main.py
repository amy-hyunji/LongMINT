...
def format_question_for_qa(question: str, qa_type, candidates) -> str:
    """Append answer-shape instructions (candidate list / ordering note) to
    a unified-dataset question before it goes to the QA model. Mirrors the
    logic used by memagent/atommem/fullcontext/hipporag so the same horizonbench
    candidate is enforced and ordering answers are emitted as a comma list."""
    is_ordering = qa_type == "ordering"
    parts = [question]
    if candidates:
        parts.append(
...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="LOCOMO", choices=['squad', 'squad_test', 'hotpotqa', 'booksum', 'friends', 'wos46985', 'pubmed-rct', 'arxiv-classification', 'eurlex', 'accurate_retrieval', 'long_range_understanding', 'conflict_resolution', 'test_time_learning', "LOCOMO", "LongMemEval", "MemAgent_Bench", "memalpha", "memalpha_train", 'memalpha_sample', "detectiveqa", 'memoryagentbench', 'perltqa', 'narrativeqa', 'accurate_retrieval', 'test_time_learning', 'cr_train', 'unified', 'unified_babi', 'unified_github', 'unified_horizonbench', 'unified_wiki']) # Restricted choices
    ...
    parser.add_argument("--sample_size", type=int, default=None, help="Only run over this many instances from the dataset")
    parser.add_argument("--max_chunks_per_instance", type=int, default=None, help="If set, keep only the first N context chunks per instance. Used for smoke tests.")
    parser.add_argument(
        "--exclude_memory",
        nargs='*',
        default=[],
...
def run_with_chunks_and_questions_batch(
    ...
    for i in range(batch_size):
        ...
        for item in queries_and_answers:
            if args.dataset == "LOCOMO":
                ...
                # 5-tuple carries an extras dict (qa_type + candidates) so we
                # can attach ordering / candidate-list instructions later. Fall
                # back to a 4-tuple shape for older parquets that pre-date this.
                if len(item) >= 5:
                    question_idx, question, answer, data_source, extras = item
                else:
    ...
    if agent_config.get('infer_with_full_memory', False) and agent_config.get('external_model_url'):
        ...
        for i, meta in enumerate(question_metadata):
            ...
            # Attach candidate-list / ordering instructions when the unified
            # question metadata carries them (horizonbench candidates,
            # ordering qa_type, etc.). For datasets without these fields it's
            # a no-op so other backends are unaffected.
            question = format_question_for_qa(
                question, meta.get('qa_type'), meta.get('candidates'),
            )
...
def main():
    ...
    # babi_facts_per_chunk: yaml-driven packing for the unified bAbI source.
    # Default 1 = legacy behavior (one fact per chunk); set in yaml (e.g.
    # memalpha_unified.yaml ships 15) to match memagent / atommem runners.
    babi_facts_per_chunk = int(agent_config.get('babi_facts_per_chunk', 1))
    conversation_creator = ConversationCreator(
        args.dataset, args.chunk_size,
        babi_facts_per_chunk=babi_facts_per_chunk,
    )
    ...
    if args.sample_size is not None:
        all_indices = all_indices[:args.sample_size]
        print(f"Sampling {args.sample_size} instances.")
    for i in range(0, len(all_indices), args.batch_size):
        ...
        if args.max_chunks_per_instance is not None:
            batch_chunks = [c[: args.max_chunks_per_instance] for c in batch_chunks]
